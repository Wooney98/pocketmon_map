...
def post_list(requests):
    #페이지 받아오기
    page=requests.GET.get('page','1')#page의 값을 get방식으로 가져오는데 없을시 디폴트를 '1'로 지정한다.

    #조회
    posts = Pocket.objects.order_by('-created_date')
    ...
    return render(requests, 'page.html', {'posts': posts, "page_range":page_range, 'max_index': max_index,'page_size':page_size})